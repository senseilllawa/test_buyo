...
const TrafficRow = ({
  ...
  return (
    <TableRow className={isWhiteRow ? "whiteRow" : "defaultRow"}>
      ...
      {visibleColumns.includes("first.unprocessed_percentage") && <TableCell className={bg("green")}>{renderThree(row.first.unprocessed_percentage, row.second.unprocessed_percentage, row.third?.unprocessed_percentage, "%")}</TableCell>}
